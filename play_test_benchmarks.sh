...

{
	...
	for start in 1 23 45; do
	    mkdir -p $output_dirname/$start 2>/dev/null
...
} | xargs -n 5 -P `grep processor /proc/cpuinfo | wc -l` bash -c 'python3 model_play_game.py --model $0 --start_level $1 --stop_level $2 2>/dev/null 1>'$output_dirname'/$1/$4.$3.log'
